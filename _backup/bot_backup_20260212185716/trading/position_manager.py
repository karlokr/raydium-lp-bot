...
class Position:
    ...
    def update_metrics(self, current_price: float, pool_data: Dict):
        ...
        if self.entry_price_ratio > 0 and self.current_price_ratio > 0:
            self.current_il_percent = analyzer.calculate_impermanent_loss(
        ...
        self.fees_earned_sol = analyzer.estimate_fees_earned(
            pool_data,
...
class PositionManager:
    ...
    def open_position(
        ...
        token_a_amount = token_a_value / current_price
    ...
    def get_total_deployed_capital(self) -> float:
        """Get total capital currently deployed in SOL"""
    ...
    def get_summary(self) -> Dict:
        ...
        avg_il = sum(pos.current_il_percent for pos in self.active_positions.values()) / len(self.active_positions) if self.active_positions else 0
...
if __name__ == "__main__":
    ...
    pos = manager.open_position(test_pool, available_capital=0.5, current_price=100.5)
    
    if pos:
        time.sleep(1)
        pos.update_metrics(current_price=102.0, pool_data=test_pool)
        
        print(f"\nPosition Status:")
        print(f"  IL: {pos.current_il_percent:.2f}%")
        print(f"  Fees: {pos.fees_earned_sol:.4f} SOL")
        print(f"  P&L: {pos.unrealized_pnl_sol:.4f} SOL")
