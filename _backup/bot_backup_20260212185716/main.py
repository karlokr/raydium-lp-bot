...
from bot.analysis.price_tracker import HybridPriceTracker
...
class LiquidityBot:
    def __init__(self):
        ...
        self.price_tracker = HybridPriceTracker(self.api_client)
        ...
        # Capital is always the current WSOL balance in the wallet
        ...
        print("=" * 60)
        print("Raydium Liquidity Bot Initialized")
        print("=" * 60)
        ...
        print(f"Position Size: {config.MAX_POSITION_PERCENT*100:.0f}% max")
    ...
    def _refresh_balance(self):
        """Refresh available capital from actual WSOL wallet balance"""
    ...
    def scan_and_rank_pools(self) -> list:
        ...
        print(f"  Found {len(pools)} qualifying pools")
        print(f"  Safe pools after quality check: {len(safe_pools)}")
        if top_pools:
            print(f"  Top pool: {top_pools[0]['name']} (score: {top_pools[0]['score']:.1f})")
